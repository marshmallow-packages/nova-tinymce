...
function cropImage(image_name) {
    performLfmRequest("crop", { img: image_name }).done(hideNavAndShowEditor);
}

function resizeImage(image_name) {
    performLfmRequest("resize", { img: image_name }).done(hideNavAndShowEditor);
}
...
function useFile(file_url) {
    ...
    var is_fcke =
        typeof data != "undefined" && data["Properties"]["Width"] != "";
